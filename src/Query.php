 ...
 * @property \src\Queries\Meta    $meta_query
 * @property \src\Queries\Date    $date_query
 * @property \src\Queries\Compare $compare_query
...
class Query extends Base {
	 ...
	 * @var   string
	 */
	protected $table_schema = '\\SwiftDB\\Schema';
	 ...
	 * @var   mixed
	 */
	protected $item_shape = '\\SwiftDB\\Row';
	 ...
	 *
	 * @since 1.0.0
	 * @var   object|\src\Queries\Compare
	 */
	 ...
	 * - bigint: Large integer, capable of storing values typically from -9,223,372,036,854,775,808 to
	 ...
	 * - real: Synonym for double in MySQL, though it can differ in other databases. Used for double precision
	...
	private function set_items( $item_ids = array() ) {
		...
		if ( ! empty( $this->query_vars['count'] ) || ! empty( $this->query_vars['aggregate'] ) ) {
			$this->items = $item_ids;
	...
	private function set_found_items( $item_ids = array() ) {
		...
		// Handle count and aggregate queries
		if ( ! empty( $this->query_vars['count'] ) || ! empty( $this->query_vars['aggregate'] ) ) {
			// Not grouped
			if ( is_numeric( $item_ids ) && empty( $this->query_vars['groupby'] ) ) {
				$this->found_items = is_float( $item_ids )
					? floatval( $item_ids )
					: intval( $item_ids );
			}
		...
		if ( is_array( $item_ids ) && ( ! empty( $this->query_vars['number'] ) && empty( $this->query_vars['no_found_rows'] ) ) ) {
			/**
			 ...
			 * @since 1.0.0
			 */
			$found_items_query = (string) apply_filters_ref_array( $this->apply_prefix( "found_{$this->item_name_plural}_query" ), array(
				'SELECT FOUND_ROWS()',
				&$this
			) );
	 ...
	 *
	 * @return \src\Queries\Meta
	 * @since 1.0.0
	 *
	 */
	private function get_meta_query( $args = array() ) {
		return new \src\Queries\Meta( $args );
	}
	 ...
	 *
	 * @return \src\Queries\Compare
	 * @since 1.0.0
	 *
	 */
	private function get_compare_query( $args = array() ) {
		return new \src\Queries\Compare( $args );
	}
	 ...
	 *
	 * @return \src\Queries\Date
	 * @since 1.0.0
	 *
	 */
	private function get_date_query( $args = array() ) {
		return new \src\Queries\Date( $args );
	}
	...
	private function get_items() {
		...
		// Never limit, never update item/meta caches when counting or aggregating
		if ( ! empty( $this->query_vars['count'] ) || ! empty( $this->query_vars['aggregate'] ) ) {
		...
		// Cast to float if not grouping aggregates
		if ( ! empty( $this->query_vars['aggregate'] ) && empty( $this->query_vars['groupby'] ) ) {
			$item_ids = floatval( $item_ids );
		}
	...
	private function get_item_ids() {
		 ...
		 *
		 */
		$clauses = (array) apply_filters_ref_array( $this->apply_prefix( "{$this->item_name_plural}_query_clauses" ), array(
			$query,
			&$this
		) );
		...
		// Handle aggregate and count queries
		if ( ! empty( $this->query_vars['count'] ) || ! empty( $this->query_vars['aggregate'] ) ) {
	...
	private function get_order_by( $order = '' ) {
		...
		if ( ! empty( $this->query_vars['count'] ) || in_array( $this->query_vars['orderby'], array(
				'none',
				array(),
				false
			), true ) ) {
		...
		} elseif ( ! empty( $this->query_vars['aggregate'] ) ) {
			$orderby = '';
	...
	private function parse_fields( $fields = '', $alias = true ) {
		...
		// No fields
		if ( empty( $fields ) ) {
			// Get groupby names if they exist
			...
			// Handle aggregates
			if ( ! empty( $this->query_vars['aggregate'] ) && is_array( $this->query_vars['aggregate'] ) ) {
				$aggregate_sql = $this->parse_aggregate( $groupby_names );

				// If parse_aggregate returns false (validation failed), fall back to normal query
				if ( false !== $aggregate_sql ) {
					$retval = $aggregate_sql;
				}
	...
	/**
	 * Parse aggregate fields and build the aggregate SQL string.
	 *
	 * @param string $groupby_names Optional. GROUP BY names for the query.
	 *
	 * @return string The aggregate SELECT clause.
	 * @since 1.0.0
	 */
	private function parse_aggregate( $groupby_names = '' ) {
		// Create new aggregate query from the aggregate args in query_vars
		$aggregate = new \src\Queries\Aggregate( $this->query_vars['aggregate'] );

		// Get the SQL from our aggregate query
		$aggregate_sql = $aggregate->get_sql(
			$this->get_table_name(),
			$this->table_alias,
		...
		// Return with group by if present
		if ( ! empty( $groupby_names ) ) {
			return "{$groupby_names}, {$aggregate_sql['select']}";
		}

		return $aggregate_sql['select'];
	}
	...
	private function shape_items( $items = array() ) {
		 ...
		 *
		 */
		$retval = (array) apply_filters_ref_array( $this->apply_prefix( "the_{$this->item_name_plural}" ), array(
			$retval,
			&$this
		) );
	...
	public function filter_item( $item = array() ) {
		return (array) apply_filters_ref_array( $this->apply_prefix( "filter_{$this->item_name}_item" ), array(
			$item,
			&$this
		) );
	}
